...
public class FeedbackController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("feedback", FeedbackDAO.getFeedback(productId, user.getId()));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!feedbackIdStr.isEmpty()) {
			...
			FeedbackDAO.updateFeedback(new FeedbackDTO(feedbackId,user.getId(),productId,rating,content,null));
		} else {
			FeedbackDAO.createFeedback(new FeedbackDTO(0,user.getId(),productId,rating,content,null));			
		}
		request.setAttribute("orderId", orderId);
		request.getRequestDispatcher("/pages/feedback/ThanksForFeedback.jsp").forward(request, response);
