...
public class ProfileController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("POST".equalsIgnoreCase(request.getMethod())) {
            ...
            if (gender != null && !gender.isEmpty()) user.setGender(Gender.valueOf(gender.toUpperCase()));
            ...
            if (password != null && !password.isEmpty()) {
                if (password.equals(confirmPassword) && password.length() >= 5) {
                    ...
                    request.setAttribute("updateSuccess", "Profile and Password Update successfully");
                } else {
                    if(!password.equals(confirmPassword)) {
                        request.setAttribute("updateError", "Password and confirm password do not match.");
                    } else if(password.length() < 5) {
                        request.setAttribute("updateError", "Password must be at least 5 characters long.");
                    }
                }
        ...
        if (!isProfileUpdated && "POST".equalsIgnoreCase(request.getMethod())) {
            request.setAttribute("updateError", "Failed to update profile. Please try again.");
        }

        request.setAttribute("categories", CategoryDAO.getCategories());
        request.setAttribute("userGender", user.getGender().toString());
